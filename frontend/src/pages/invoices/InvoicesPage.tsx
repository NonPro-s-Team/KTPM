    ...
    <div className="management-page">
      ...
      <Modal open={modalOpen} onClose={() => { if (!submitting) setModalOpen(false); }} title={editing ? "Chỉnh sửa hóa đơn nháp" : "Tạo hóa đơn nháp"} description="Điền chỉ số đầu/cuối và đơn giá; Backend sẽ tính các thành tiền." footer={<><Button variant="secondary" disabled={submitting} onClick={() => setModalOpen(false)}>Hủy</Button><Button type="submit" form="invoice-form" loading={submitting}>Lưu hóa đơn</Button></>}>
        <form id="invoice-form" className="management-form" onSubmit={handleSubmit} noValidate>
          ...
          {!editing ? <><Select label="Hợp đồng" placeholder={contractsQuery.loading ? "Đang tải hợp đồng..." : "Chọn hợp đồng"} searchable value={form.contractId} options={(contractsQuery.data?.items ?? []).filter((contract: RentalContractResponse) => contract.status === "active").map((contract) => ({ value: contract.id, label: `${contract.roomNumber ?? contract.roomId.slice(0, 8)} · ${contract.tenantName ?? contract.tenantId.slice(0, 8)}` }))} error={contractsQuery.error || fieldErrors.contractId?.[0]} helperText={(contractsQuery.data?.totalCount ?? 0) > 100 ? "Chỉ hiển thị 100 hợp đồng đầu, dùng ô tìm kiếm bên trên để lọc thêm." : undefined} required onChange={(event) => setForm((current) => ({ ...current, contractId: event.target.value }))} /><Input label="Kỳ hóa đơn" type="month" value={form.billingPeriod} error={fieldErrors.billingMonth?.[0] ?? fieldErrors.billingYear?.[0]} required onChange={(event) => setForm((current) => ({ ...current, billingPeriod: event.target.value }))} /></> : <Input label="Tiền thuê" type="number" min="0" suffix="VND" value={form.rentAmount} error={fieldErrors.rentAmount?.[0]} required onChange={(event) => setForm((current) => ({ ...current, rentAmount: event.target.value }))} />}
