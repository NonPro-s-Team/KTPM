...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {
        ...
        "jwt.secret=qlpt355-local-only-signing-secret-at-least-32-bytes",
        "app.sepay.api-key=qlpt355-local-sepay-key", "vnpay.tmn-code=EP355",
        "vnpay.hash-secret=qlpt355-local-vnpay-key", "vnpay.pay-url=http://127.0.0.1/never-open-payment",
        "vnpay.return-url=http://127.0.0.1/never-open-return"
})
@ActiveProfiles("test")
class OrderAmountEpIT {
    private static final ObjectMapper JSON = new ObjectMapper().enable(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS);
    private static final Path OUT = Path.of(System.getProperty("ep.output", "../test-evidence/order-payment-ep/generated")).toAbsolutePath().normalize();
    private static final String SCHEMA = "qlpt355_bva_" + UUID.randomUUID().toString().replace("-", "").substring(0, 12);
    private static String dbUrl, dbUser, dbPassword;
    private static final boolean MYSQL = "mysql".equals(System.getProperty("ep.database"));
    ...
    static void isolatedMysql(DynamicPropertyRegistry registry) throws Exception {
        if(!MYSQL) {
            ...
            write("test-environment.json",Map.of("schema",SCHEMA,"database","H2 MySQL mode; JPA generated schema, NOT MySQL","startedAt",Instant.now().toString(),"isolation","ephemeral in-memory DB; existing MySQL untouched","externalServices","GHN and token blacklist mocked; local signed VNPay/SePay callbacks","limitation","H2 deliberately selected for isolation; MySQL not attempted in this run"));
            return;
        }
    ...
    @Test void executeEpAndSaveEvidence() throws Exception {
        ...
        List<Scenario> cases = List.of(
            new Scenario("EP01-positive-no-promo", "75000", "", "0", false,false,"0","30000","105000"),
            new Scenario("EP02-carrier-fee", "75000", "", "0",false,true,"0","19000","94000"),
            new Scenario("EP03-fixed-partial", "75000", "FIXED", "20000",false,false,"20000","30000","85000"),
            new Scenario("EP04-fixed-exceeds-subtotal", "75000", "FIXED", "120000",false,false,"75000","30000","30000"),
            new Scenario("EP05-zero-total", "75000", "FIXED", "75000",true,false,"75000","0","0"),
            new Scenario("EP06-percent-fraction", "75321", "PERCENT", "15",true,false,"11298","0","64023"),
            new Scenario("EP07-percent-full", "75000", "PERCENT", "100",true,false,"75000","0","0"),
            new Scenario("EP08-positive-decimal", "75321.45", "FIXED", "20000",true,false,"20000","0","55321.45")
        );
        try {
            for (String flow : List.of("BUY", "CART")) for (Scenario c : cases) runOrder(flow,c);
            supplemental();
            invalidEpInputs();
            results.add(Map.of("id","MYSQL-STORAGE", "status","Blocked", "expected","Verify signed DECIMAL(12,2), NULL and overflow on MySQL", "actual","This run uses isolated H2; actual MySQL not tested."));
        ...
        if(Boolean.getBoolean("ep.failOnFindings")) assertTrue(checks.stream().allMatch(c->c.get("status").equals("Pass")),"Confirmed product findings: see test-results.json and assertion-results.json; all evidence was saved before this assertion");
    }

    private void seed() {
        if(!MYSQL) em.createNativeQuery("alter table orders alter column shipping_address varchar(10000)").executeUpdate();
        User user = User.builder().name("QLPT355 customer").email("customer@qlpt355.example.test").hasPassword(false).role(User.Role.CUSTOMER).isActive(true).build();
        em.persist(user); userId=user.getId(); token=jwt.generateAccessToken(userId,"CUSTOMER");
        User admin=User.builder().name("QLPT355 admin").email("admin@qlpt355.example.test").hasPassword(false).role(User.Role.ADMIN).isActive(true).build();
    ...
    private void runOrder(String flow,Scenario c) throws Exception {
        ...
        Map<String,Object> b=requestFor(flow,c.price(),"VNPAY");b.put("note","QLPT-355 "+id);
        ...
        if(o.has("id")) {
            ...
            if(bd(c.expectedTotal()).signum()>0) {
                for(int delta:List.of(-2500000,2500000)) {
    ...
    private void supplemental() throws Exception {
        ...
        for(String method:List.of("BANK_TRANSFER","MOMO")) for(int delta:List.of(-5000,0,5000)) {
            ...
            Map<String,Object>body=Map.of("transferType","in","transferAmount",bd("80000").add(BigDecimal.valueOf(delta)),"content",o.path("orderCode").asText(),"referenceCode",id);
            actual=call(id,"POST","/api/webhooks/sepay",body,"Apikey qlpt355-local-sepay-key",200);Map<String,Object>db=dbOrder(oid);String expected=delta==0?"PAID":"PENDING";check(id,"SePay threshold",expected.equals(db.get("payment_status")),expected,db.get("payment_status"));check(id,"payment amount unchanged",bd(db.get("amount").toString()).compareTo(bd("80000"))==0,"80000",db.get("amount"));results.add(result(id,start,Map.of("expected",expected+"; amount remains 80000", "actual",actual,"input",body,"db",db)));
        }
        for(String flow:List.of("BUY","CART")) {
    ...
    private void invalidEpInputs() throws Exception {
        for(String flow:List.of("BUY","CART")) {
            String id="EP10-valid-quantity-"+flow;int start=checks.size();
            Map<String,Object>b=requestFor(flow,"75000","COD");
            if(flow.equals("BUY")) b.put("quantity",3);
            else tx.executeWithoutResult(t -> em.createQuery("update CartItem c set c.quantity=3 where c.variant.id=:v").setParameter("v",variantId).executeUpdate());
            JsonNode actual=call(id,"POST",flow.equals("BUY")?"/api/orders/buy-now":"/api/orders",b,token,200);
            money(id,"total for quantity 3","255000",actual.path("totalAmount"));
            Map<String,Object>db=dbOrder(actual.path("id").asLong());
            check(id,"payment equals order",bd(db.get("amount").toString()).compareTo(bd("255000"))==0,"255000",db.get("amount"));
            results.add(result(id,start,Map.of("input",b,"quantity",3,"expected","subtotal=225000; shipping=30000; total=payment=255000","actual",actual,"db",db)));
        }
        for(String kind:List.of("missing", "fraction", "text", "over-stock")) {
            String id="EP09-quantity-"+kind; int start=checks.size();
            Map<String,Object> b=requestFor("BUY","75000","COD");
            switch(kind){case "missing" -> b.remove("quantity");case "fraction" -> b.put("quantity",1.5);case "text" -> b.put("quantity","abc");default -> b.put("quantity",1500);}
            Map<String,Object> before=snapshot();JsonNode actual=call(id,"POST","/api/orders/buy-now",b,token,400);
            check(id,"no mutation",before.equals(snapshot()),before,snapshot());
            results.add(result(id,start,Map.of("input",b,"expected","HTTP 400; orders/payments/stock unchanged","actual",actual,"before",before,"after",snapshot())));
        }
    }
    ...
    private Map<String,Object> promotion(String code,String type,String value,boolean free) {
        return Map.of("code",code,"name","QLPT355 "+code,"type",type,"value",bd(value),"minOrderValue",0,"target","PUBLIC","freeShipping",free,"startsAt",LocalDateTime.now().minusDays(1).toString(),"endsAt",LocalDateTime.now().plusDays(2).toString(),"isActive",true);
    }
    private JsonNode callback(String id,String route,JsonNode order,String amount,boolean valid) throws Exception {
        Map<String,String> p=new TreeMap<>(Map.of("vnp_TmnCode","EP355","vnp_TxnRef",order.path("orderCode").asText(),"vnp_Amount",amount,"vnp_ResponseCode","00","vnp_TransactionStatus","00","vnp_TransactionNo","355"+order.path("id").asText()));
        String query=query(p);Mac mac=Mac.getInstance("HmacSHA512");mac.init(new SecretKeySpec("qlpt355-local-vnpay-key".getBytes(StandardCharsets.UTF_8),"HmacSHA512"));
